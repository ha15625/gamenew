    ...
    async getLookUps(req, res, next) {
        ...
        try {
            ...
            if (isDetailExist?.investments) {
                console.log("Hello ...")
                my_object.isDetailExist?.investments = 'Investments';
                my_object.flag = true;
                availableDataPoints.push(my_object);
            }
            if (isDetailExist?.homeInfo) {
                console.log("2 ...")
                my_object.isDetailExist?.homeInfo = 'Home';
                my_object.flag = true;
                availableDataPoints.push(my_object);
            }
            if (isDetailExist?.donor) {
                console.log("3 ...")
                my_object.isDetailExist?.donor = 'Donor';
                my_object.flag = true;
                availableDataPoints.push(my_object);
            }
            if (isDetailExist?.finance) {
                ...
                my_object.isDetailExist?.finance = 'Financial';
                my_object.flag = true;
                availableDataPoints.push(my_object);
            }
