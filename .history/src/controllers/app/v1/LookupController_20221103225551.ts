...
class LookupController {
    ...
    async getLookUps(req, res, next) {
        const { service, url, userId } = req.body
        try {
            ...
            let testNewData = [{ dataPoint: availableDataPoints }]
            const expandData = await ExpandLimit.find({ userId: userId }).populate("linkedinUrl")

            const newArray = testNewData.concat(expandData)

            let allFeaturs = [...testNewData, ...expandData];


            console.log(allFeaturs)
            return _RS.ok(res, "SUCCESS", "Success!", newArray)
        } catch (err) {
            next(err);
        }
    }
